...
public class EnergyConsumptionAnalyzer extends P2CP1Analyzer {
	...
	protected void runAction() {
		...
		if (!(currentInstruction instanceof ChargeInstruction)) {
			...
			EnergyConsumptionPredictor predictor = new EnergyConsumptionPredictor(getModels().getEnvMapModel().getModelInstance(), getModels().getMissionStateModel().getModelInstance(),m_powerModel);
			predictor.setConfig(getModels().getRobotStateModel().getModelInstance().getConfigId());
			while (it.hasNext ()) {
				if (first) {
					first = false;
					hasEnoughEnergy &= predictor.getPlanEnergyConsumption(currentInstruction, it.next(), tgtWP) < batteryCharge;
				}
				else {
					hasEnoughEnergy &= predictor.getPlanEnergyConsumption(null, it.next(), tgtWP) < MapTranslator.ROBOT_BATTERY_RANGE_MAX;
				}
			}
