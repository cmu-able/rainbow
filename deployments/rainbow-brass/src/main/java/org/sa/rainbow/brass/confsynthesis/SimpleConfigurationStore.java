...
public class SimpleConfigurationStore implements ConfigurationProvider {
	...
	public SimpleConfigurationStore(Properties props) {
		m_source = ConfigHelper
				.convertToAbsolute(props.getProperty(PropertiesSimpleConfigurationStore.CONFIGURATIONS_SOURCE_PROPKEY));
		// m_source =
		// "/Users/jcamara/Dropbox/Documents/Work/projects/rainbow-alt/deployments/rainbow-brass/prismtmp/p2cp1/config_list.json";
	}
	...
	private synchronized void loadFromFile(String confFile) {
		...
		for (Object node : nodes) {
			...
			try {
				...
				if (c_cdr < 0 || c_speed <= 0) {
					System.out
							.println("Error: data doesn't make any sense - power_load=" + c_cdr + ", speed=" + c_speed);
				}
			} catch (Exception e) {
			...
			System.out.println("Added configuration " + m_configuration_objects.get(m_conf_prefix + c_id).getId()
					+ " - Discharge rate: "
