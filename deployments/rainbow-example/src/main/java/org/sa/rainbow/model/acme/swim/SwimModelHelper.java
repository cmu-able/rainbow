...
public class SwimModelHelper {
	...
	private static final String PROP_AVG_UTILIZATION = "[EXPR]seqAverage(</self/components:ServerT[isArchEnabled and isActive]/load>)";
	...
	public double getAvgUtilization() {
		...
		/*
		 * PROP_AVG_UTILIZATION gets the average over all the servers in the model
		 * The load for inactive servers will be 0, so we have to compensate to get
		 * the average over the active servers
		 */
		return getDoubleProperty(PROP_AVG_UTILIZATION, 0.0);
	}
}
