...
public class WorkerThreadGroup implements WorkerThreadGroupCI {
	...
	public synchronized void add_thread(WorkerThread wt) {
		Ensure.notNull(wt);
		Ensure.stateCondition(!m_threads.contains(wt));
		m_threads.add(wt);
	}
	...
	public synchronized void add_subgroup(WorkerThreadGroup wtg) {
		Ensure.notNull(wtg);
		Ensure.isTrue(wtg != this);
		Ensure.stateCondition(!m_subgroups.contains(wtg));
		Ensure.stateCondition(!wtg.all_subgroups().contains(this));
		m_subgroups.add(wtg);
	}
