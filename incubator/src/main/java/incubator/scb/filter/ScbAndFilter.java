...
/**
 * SCB filter that performs an <em>and</em> of two filters.
 * @param <T> the data type to filter
 */
public class ScbAndFilter<T> implements ScbFilter<T> {
	/**
	 * The first filter.
	 */
	 ...
	 */
	public ScbAndFilter(ScbFilter<T> f1, ScbFilter<T> f2) {
	 ...
	 * If both are non-<code>null</code>, returns an <em>and</em> filter
	 * with both filtrers.
	 * @param f1 the first filter
	 * @param f2 the second filter
	 * @return the filter
