function Gauge(placeholderName, configuration) {
	...
	this.render = function() {
		...
		if (typeof this.placeholderName == "string") {
			...
			.attr("class", "gauge").attr("width", this.config.size).attr(
					"height", this.config.size);
		}
		else {
			this.body = d3.select (this.placeholderName).append ("svg:svg").attr("class", "gauge").
			attr ("width", this.config.size).attr ("height", this.config.size);
		}
		

		this.body.append("svg:circle").attr("cx", this.config.cx).attr("cy",
				this.config.cy).attr("r", this.config.raduis).style("fill",
				"#ccc").style("stroke", "#000").style("stroke-width", "0.5px");

		this.body.append("svg:circle").attr("cx", this.config.cx).attr("cy",
				this.config.cy).attr("r", 0.9 * this.config.raduis).style(
				"fill", "#fff").style("stroke", "#e0e0e0").style(
				"stroke-width", "2px");
		...
		if (undefined != this.config.label) {
			...
			this.body.append("svg:text").attr("x", this.config.cx).attr("y",
					this.config.cy / 2 + fontSize / 2).attr("dy", fontSize / 2)
					.attr("text-anchor", "middle").text(this.config.label)
					.style("font-size", fontSize + "px").style("fill", "#333")
		...
		for (var major = this.config.min; major <= this.config.max; major += majorDelta) {
			...
			this.body.append("svg:line").attr("x1", point1.x).attr("y1",
					point1.y).attr("x2", point2.x).attr("y2", point2.y).style(
			...
			if (major == this.config.min || major == this.config.max) {
				...
				this.body.append("svg:text").attr("x", point.x).attr("y",
						point.y).attr("dy", fontSize / 3).attr("text-anchor",
						major == this.config.min ? "start" : "end").text(major)
						.style("font-size", fontSize + "px").style("fill",
		...
		pointerContainer.append("svg:circle").attr("cx", this.config.cx).attr(
				"cy", this.config.cy).attr("r", 0.12 * this.config.raduis)
				.style("fill", "#4684EE").style("stroke", "#666").style(
						"opacity", 1);

		var fontSize = Math.round(this.config.size / 10);
		pointerContainer.selectAll("text").data([ midValue ]).enter().append(
				"svg:text").attr("x", this.config.cx).attr("y",
				this.config.size - this.config.cy / 4 - fontSize).attr("dy",
				fontSize / 2).attr("text-anchor", "middle").style("font-size",
				fontSize + "px").style("fill", "#000").style("stroke-width",
				"0px");
	...
	this.drawBand = function(start, end, color) {
		...
		this.body.append("svg:path").style("fill", color).attr(
				"d",
				d3.svg.arc().startAngle(this.valueToRadians(start)).endAngle(
						this.valueToRadians(end)).innerRadius(
...
window.edu_cmu_rainbow_ui_display_widgets_Tachometer = function() {
	...
	this.createGauge = function(name, label, min, max, red, yellow, editable) {
		...
		config.redZones = [ {
			from : config.min + range * tRed,
			to : config.max
		} ];
		config.yellowZones = [ {
			from : config.min + range * tYellow,
			to : config.redZones[0].from
		} ];

		gauge = new Gauge(element, config);
		gauge.render();
	}
}