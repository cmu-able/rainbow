...
public class StitchScriptEvaluator extends BaseStitchBehavior {
	...
	private Tactic m_executingtactic;
	...
	public void beginTactic(Token nameAST) {
		m_executingtactic = (Tactic )scope ();
	}
	
	@Override
	public void endTactic() {
		m_executingtactic = null;
	}
	...
	public void createVar(StitchParser.DataTypeContext type, TerminalNode id, StitchParser.ExpressionContext val,
		...
		if (scope().expressions().size() > 0) {
			...
			if (v != null && v.valStmt != null) {
				...
				if (m_executingtactic != null && m_executingtactic.state == Tactic.ParseState.IN_VARS) {
					// Set up the pre value
					Var preV = (Var) scope().lookup("__$pre$__" + id.getText());
					if (preV != null) {
						preV.setValue(e.getResult());
					}
				}
			}
		}
	...
	@Override
	public void doQuantifiedExpression(Strategy.ExpressionKind type, StitchParser.QuantifiedExpressionContext ctx) {
		if (checkSkipEval())
			return;
	...
	@Override
	public void endQuantifiedExpression(Strategy.ExpressionKind quant,
			StitchParser.QuantifiedExpressionContext quantifiedExpressionContext) {
		if (checkSkipEval())
			return;
	...
	public void doLogicalExpression(Strategy.ExpressionKind opAST, ParserRuleContext ctx) {
		...
		Boolean lOp = null;
		Boolean rOp = null;
		if (expr.lrOps[LOP].isEmpty() || expr.lrOps[LOP].peek() == null || expr.lrOps[ROP].isEmpty()
			...
			final String msg = "One logical operand is NULL: "
					...
					+ (expr.lrOps[ROP] == null || expr.lrOps[ROP].isEmpty() ? "NULL" : expr.lrOps[ROP].pop());
		...
		if (expr.lrOps[LOP].peek() instanceof Boolean && expr.lrOps[ROP].peek() instanceof Boolean) {
			...
			Tool.error("Type mismatch or NOT booleans in logical expression! " + expr.lrOps[LOP].peek() + " "
	...
	public void doArithmeticExpression(Strategy.ExpressionKind kind, ParserRuleContext opAST) {
		...
		if (expr.lrOps[LOP].peek() instanceof MyNumber && expr.lrOps[ROP].peek() instanceof MyNumber) {
			...
			Tool.error("Types not Numbers in arithmetic expression! " + expr.lrOps[LOP].peek() + " " + opAST.getText()
					+ " " + expr.lrOps[ROP].peek(), opAST, stitchProblemHandler());
			return;
		}
	...
	public void doPostIdentifierExpression(StitchParser.PostIdExpressionContext identifier) {
		...
		if (o instanceof PostVar) {
			...
			// This is not necessarily true because of the following:
			// int s = 0
			//
			ParseTreePattern p = stitch().parser.compileParseTreePattern("<dataType> <IDENTIFIER> = <idExpression>",
					StitchParser.RULE_var);
			
			ParseTreeMatch m = p.match(v.preVar().valStmt.tree());
			if (!m.succeeded()) {
				v.computeAndSetValue();
				expr.setResult(((Var) o).getValue());
			} 
			else {
				expr.setResult(v.preVar().getValue());
			}
		} else {
			expr.setResult(null);
			stitchProblemHandler().setProblem(generateErrorFromToken(identifier.IDENTIFIER().getSymbol(),
					"Variable " + identifier.getText() + " not defined."));
		}
	}
	...
	public void doIdentifierExpression(ParserRuleContext idAST, Strategy.ExpressionKind kind) {
		...
		if (kind == Strategy.ExpressionKind.IDENTIFIER) {
			...
			if (o == null && m_executingtactic != null && m_executingtactic.state == Tactic.ParseState.IN_EFFECT) {
				o = scope().lookup("__$pre$__" + iden);
			}
			if (o == null)
				o = scope().lookup(iden);
			...
			// System.out.println ("StitchScriptEvaluator:doArithmeticExpression setting
			// expr result to null");
			expr.setResult(null);
		}
	}
	...
	/**
	 * Executes a method assuming Java... can be refactored and extended later to
	 * support methods in other languages.
	 *
	 * @param name
	...
	boolean checkMethodParams(Method m, Class[] params, Object[] args) {
		...
		for (Class c : m.getParameterTypes()) {
			...
			if (c.isAssignableFrom(params[i])) { // good!
				...
				matchPrimitive = c.equals(int.class) && Integer.class.equals(params[i])
	...
	private Object executeDesignAnalysis(String name, Object[] args, IAcmeDesignAnalysis da) {
		...
		try {
			...
			if (da.getAnalysisType() == IAcmeDesignAnalysis.DesignAnalysisType.EXTERNAL) {
				if (context == null) {
					Tool.error(MessageFormat.format("Could not find the object ''{0}'' to do the call",
				...
				result = RuleTypeChecker.evaluateAsType(context, returnType, null, da.getExpression(), errors, lookup);
			}
