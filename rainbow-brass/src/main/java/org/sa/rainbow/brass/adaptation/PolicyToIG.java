...
public class PolicyToIG {
     ...
     *            double linear speed
     * @param theta
     *            orientation angle (in radians) that the robot should face after moving
     * @return
     */
    ...
    /**
     * Finds the orientation of the next movement in plan (to indicate orientation of robot at the end of current
     * MoveAbsH movement)
     * 
     * @param plan
     * @param index
    ...
    public double findNextOrientation (ArrayList<String> plan, int index) {
        ...
        if (index + 1 >= plan.size ()) return theta;
        ...
        synchronized (m_map) {
            ...
            if (src_x == Double.NEGATIVE_INFINITY || src_y == Double.NEGATIVE_INFINITY) return theta;
        ...
        for (EnvMapNode node_src : map.getNodes ().values ()) {
            for (EnvMapNode node_tgt : map.getNodes ().values ()) {
                if (node_src.getId () != node_tgt.getId ()) {
                    ...
                    PrismPolicy prismPolicy = new PrismPolicy (
