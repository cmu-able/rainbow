...
fact tacticOrdering {
	...
	DecDimmer.next = MinDimmer
	MinDimmer.next = MidDimmer
	MidDimmer.next = MaxDimmer
}
...
fact traces {
	...
	all e : TraceElement - last | let e' = next[e] | {
		...
		equals[e, e'] 
		equals[e', trace/last]
	} or 	((addServerTacticStart[e, e']  or removeServerTactic[e, e'] or decDimmerTactic[e, e'] or incDimmerTactic[e, e'] or
				maxDimmerTactic[e, e'] or minDimmerTactic[e, e'] or midDimmerTactic[e, e']) and
...
pred incDimmerCompatible[e : TraceElement] {
	!(IncDimmer in e.starts) and !(DecDimmer in e.starts) and 
		!(MinDimmer in e.starts) and !(MaxDimmer in e.starts) and 	!(MidDimmer in e.starts)
}
...
pred maxDimmerCompatible[e : TraceElement] {
	!(IncDimmer in e.starts) and !(DecDimmer in e.starts) and 
		!(MinDimmer in e.starts) and !(MaxDimmer in e.starts) and 	!(MidDimmer in e.starts)
}

pred maxDimmerTactic[e, e' : TraceElement] {
	maxDimmerCompatible[e] and e.cp.d != dimmer/last
	e'.starts = e.starts + MaxDimmer
...
pred minDimmerCompatible[e : TraceElement] {
	!(IncDimmer in e.starts) and !(DecDimmer in e.starts) and 
		!(MinDimmer in e.starts) and !(MaxDimmer in e.starts) and 	!(MidDimmer in e.starts)
}

pred minDimmerTactic[e, e' : TraceElement] {
	minDimmerCompatible[e] and e.cp.d != dimmer/first
	e'.starts = e.starts + MinDimmer

	let c = e.cp, c'=e'.cp | {
		c'.d = (dimmer/first) //.next.next.next.next
...
pred midDimmerTactic[e, e' : TraceElement] {
	minDimmerCompatible[e] and e.cp.d != midDimmerSetting
	e'.starts = e.starts + MidDimmer

	let c = e.cp, c'=e'.cp | {
		c'.d = midDimmerSetting
...
fun midDimmerSetting: one D { { d : D | #(d.prevs) = #(d.nexts) or 
	(d != last and #(d.next.prevs) = #(d.nexts)) }}

assert c1 {
	midDimmerSetting = (dimmer/first)
}
check  c1 for exactly 3 S, exactly 3 TAP, 2 D, 3 C, 3 CP, 3 TraceElement
