...
// will have completed in the next step.open util/ordering[F] as FO
...
pred incAltTacticProgress[c, c' : CP] {
	...
	c'.p[IncAlt] = TPIAO/last // if it was running, it's complete now

	// nothing else changes other than a and the progress
...
pred decAltTacticProgress[c, c' : CP] {
	...
	c'.p[DecAlt] = TPDAO/last  // if it was running, it's complete now
