    ...
    public DataValue from_java (Object value, DataType dst, TypelibJavaConverter converter)
        ...
        if ((dst == null || dst instanceof StructureDataType) && value instanceof IRainbowModelCommandRepresentation) {
            try {
                ...
                fields.put (label, converter.from_java (command.getLabel (), ESEBProvider.SCOPE.string ()));
                fields.put (target, converter.from_java (command.getTarget (), ESEBProvider.SCOPE.string ()));
                fields.put (modelName, converter.from_java (command.getModelName (), ESEBProvider.SCOPE.string ()));
                fields.put (modelType, converter.from_java (command.getModelType (), ESEBProvider.SCOPE.string ()));
                fields.put (commandName, converter.from_java (command.getCommandName (), ESEBProvider.SCOPE.string ()));
                fields.put (params, converter.from_java (Arrays.asList (command.getParameters ()), null));
