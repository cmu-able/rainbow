...
import org.sa.rainbow.gauges.GaugeState;
    ...
    public DataValue from_java (Object value, DataType dst, TypelibJavaConverter converter)
        ...
        if ((dst == null || dst instanceof StructureDataType) && value instanceof IGaugeState) {
            try {
                ...
                if (sdt == null) {
                    sdt = (StructureDataType )ESEBProvider.SCOPE.find ("command_representation");
                }
                ...
                fields.put (
                        setup,
                        converter.from_java (command.getSetupParams (),
                                new ListDataType (ESEBProvider.SCOPE.find ("typed_attribute_with_value"), ESEBProvider.SCOPE
                                        .any ())));
                fields.put (config, converter.from_java (command.getConfigParams (), new ListDataType (ESEBProvider.SCOPE.find ("typed_attribute_with_value"), ESEBProvider.SCOPE
                        .any ())));
                fields.put (commands, converter.from_java (command.getGaugeReports (), new ListDataType (
                        ESEBProvider.SCOPE.find ("command_representation"), ESEBProvider.SCOPE.any ())));
    ...
    public <T> T to_java (DataValue value, Class<T> cls, TypelibJavaConverter converter)
        ...
        if (value instanceof StructureDataValue) {
            try {
                ...
                List setup = converter.<List> to_java (sdv.value (sdt.field ("label")), List.class);
                List config = converter.<List> to_java (sdv.value (sdt.field ("target")), List.class);
                List commands = converter.<List> to_java (sdv.value (sdt.field ("modelName")), List.class);
                if (cls == null) {
