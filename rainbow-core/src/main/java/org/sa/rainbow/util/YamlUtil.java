...
import org.sa.rainbow.gauges.GaugeDescription;
import org.sa.rainbow.gauges.GaugeInstanceDescription;
import org.sa.rainbow.gauges.GaugeTypeDescription;
...
//	}
    ...
    public static GaugeDescription loadGaugeSpecs () {
        File gaugeSpec = Util.getRelativeToPath (Rainbow.instance ().getTargetPath (), Rainbow.properties ()
                .getProperty (Rainbow.PROPKEY_GAUGES_PATH));
            ...
            for (Map.Entry<String,Map> instSpec : instanceMap.entrySet()) {
                ...
                if (gaugeTypeSpec == null) {
                    Util.LOGGER.error (MessageFormat.format (
                            "Cannot find gauge type: {0} referred to in gauge ''{1}''.",
                            gaugeType, gaugeName));
                    continue;
                }
