...
//				.join(', '))
	...
	def outputCommandFactory(Resource resource, IFileSystemAccess2 access2, Factory factory) {
	...
	def outputUtilityModel(URI uri, Iterable<DeclaredProperty> properties, EList<ImpactVector> list) {
		...
		for (um : properties) {
			...
			utilities:
			  «FOR u : us.assignment»
			    «u.name»:
			      label: «surroundString(unpackString((u.value.value as Component).assignment.findFirst[it.name=='label']?.value.value as StringLiteral, false), true)»
			      mapping: «surroundString(unpackString((u.value.value as Component).assignment.findFirst[it.name=='mapping']?.value.value as StringLiteral, false), true)»
			      description: «surroundString(unpackString((u.value.value as Component).assignment.findFirst[it.name=='description']?.value.value as StringLiteral, false), true)»
			      utility:
